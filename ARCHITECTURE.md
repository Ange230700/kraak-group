...
## Déclaration De Stack (Scope Figé)

Moment de déclaration:

- la stack est déclarée juste après le gel du scope MVP, avant setup technique détaillé et avant travaux UI.

Décision de stack mise à jour (verrouillée):

- Fondation frontend unique: workspace Angular dans `apps/client`.
- UI website: PrimeNG (marketing website dans `projects/web`).
- UI mobile: Ionic Angular (dans `projects/mobile`).
- Runtime mobile: Capacitor.
- Backend/API: NestJS (`apps/api`).
- Data/Auth/Storage: Supabase.
- Notifications: Firebase Cloud Messaging via Capacitor (ou équivalent validé).
- Analytics:
  - Website: PostHog ou Google Analytics.
  - Mobile: outil analytics mobile dédié si nécessaire au pilotage produit.
- Déploiement:
  - Website: Vercel (ou équivalent).
  - Mobile: distribution APK/TestFlight/internal testing selon phase.
- Stratégie de contenu:
  - Website: contenu statique ou gestion simple.
  - Mobile: contenu récupéré via API/backend.
