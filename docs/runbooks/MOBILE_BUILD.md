...
| JDK            | 17+                  | Temurin recommandé — nécessaire pour Gradle / Android |
...
Il faut les créer une seule fois en local à l'aide des commandes suivantes :
...
Ces dossiers doivent ensuite être commités dans le dépôt (convention Capacitor).  
Les artefacts de build intermédiaires sont déjà exclus via `apps/client/.gitignore`.
...
Pour ouvrir et builder dans Xcode, voir la section suivante.
...
```bash
pnpm dev:mobile
# URL locale : http://localhost:4300
```
...
```bash
pnpm cap:sync
pnpm cap:open:android   # ou cap:open:ios
```

4. Lancer l'app depuis Android Studio / Xcode sur un émulateur ou un appareil connecté.
...
- installe Java 17 (Temurin)
- exécute `pnpm build:mobile` puis `cap sync android`
